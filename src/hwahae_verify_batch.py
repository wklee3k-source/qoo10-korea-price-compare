...
def _clean_query(text: str) -> str:
    ...
    t = BRACKET_RE.sub("", t)
    # [개선] "+"로 여러 부속품을 나열한 세트상품(예: "로션+피니시로션",
    # "본체+리필")은 검색어가 길고 복잡해져서 검색엔진이 못 찾는 경우가
    # 많았다(실측: 정제 후에도 180건에서 "+"가 남아있었음). 대표
    # 상품(첫 부분)만 남긴다 — 세트 전체가 아니라 핵심 성분/타입으로
    # 검색해야 실제 판매 페이지를 찾을 확률이 높다.
    if "+" in t:
        t = t.split("+")[0].strip()
    # [개선] "/"로 구분된 효능/키워드 나열(예: "/콜라겐/광채/고영양/활력/탄력")도
    # 마찬가지로 검색어를 오염시킨다. "/"가 2개 넘게 있으면(=3구간 이상=
    # 나열형일 가능성 높음) 첫 구간만 남긴다.
    if t.count("/") >= 2:
        t = t.split("/")[0].strip()
