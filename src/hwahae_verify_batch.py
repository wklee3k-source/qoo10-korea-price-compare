...
# [v7.57.0] 큐텐 판매자가 검색 노출을 노리고 붙인 군더더기.
#
# [왜 빼는가 — 실측 2026-08-15, 재검증 2,006건 중간 점검]
# 버려진 건과 성공한 건을 갈라 보니 검색어의 잡스러움이 갈랐다.
#   · 군더더기 2개 이상 포함: 버려진 건 12.1% / 성공한 건 4.4% (3배)
#   · 평균 상품명 길이:      버려진 건 57자   / 성공한 건 39자
#
# 실제로 버려진 것 중에 이런 게 있었다 — 전부 우리가 팔아야 할 물건이다.
#   "달바 워터풀 톤업 자외선 차단제 그린 50ml 소녀시대 태연 걸스데이 혜리"
#   "포맨트 방탄소년단 정국 향수 ... 코튼 허그 블랙 50ml 1개[정국/한국화장품"
#   "센텔리안24 [정품] [구] 센텔리안 MD 크림 / [신] 마데카 MD 크림 / 한국"
# 연예인 이름과 홍보 문구가 그대로 검색어로 들어가 소스가 엉뚱한 걸
# 물어왔다. 앞의 두 건은 아예 아무 소스도 응답하지 못했다.
#
# [주의] 제품명의 일부일 수 있는 말은 넣지 않는다. 예를 들어 "세트"는
# 실제 상품명에 흔히 들어가고("3종 세트"), "쿠션"은 제형이다. 확실한
# 홍보·인명만 넣는다 — 잘못 지우면 오히려 못 찾는다.
_PROMO_WORDS = [
    # 연예인·그룹 (판매자가 붙이는 검색 미끼)
    "소녀시대", "방탄소년단", "블랙핑크", "뉴진스", "아이브", "에스파",
    "아이유", "태연", "정국", "지민", "제니", "지수", "리사", "로제",
    "혜리", "걸스데이", "수지", "차은우", "손흥민",
    # 홍보 문구
    "최저가", "무료배송", "당일발송", "국내발송", "해외직구", "정품보장",
    "베스트", "신상품", "인기상품", "추천상품", "이벤트", "특가", "할인중",
    "품절임박", "재입고", "한정판매", "단독판매", "사은품증정", "리뷰이벤트",
    # 출처 표기
    "쿠텐", "큐텐", "한국화장품", "한국 화장품", "한국코스메", "한국 코스메",
    "한국스킨케어", "올리브영구매대행", "구매대행",
    # 큐텐 자체 행사명 — 상품명이 아니라 판촉 문구다.
    # [08회차 번역 피드백, 2026-08-15] 번역하는 쪽이 "메가와리는 큐텐재팬
    # 프로모션명(메가할인 행사)이라 상품명이 아닌데 그대로 옮겼다,
    # 검증에 영향 줄 수 있다"고 알려줬다. 실제로 이런 말이 검색어에
    # 들어가면 어느 쇼핑몰에서도 안 잡힌다.
    "메가와리", "메가할인", "메가포", "타임세일", "쿠폰가능", "쿠폰발행",
    "리뷰캠페인", "리뷰이벤트", "샵쿠폰", "득템",
    # [09-07 번역 피드백] 일본어 원문 태그도 걸러야 한다.
    #
    # 한글로 옮겨진 "한국화장품"·"한국코스메"는 이미 걸러지는데,
    # 원문 그대로 남은 `韓国化粧品`·`韓国コスメ`는 안 걸러졌다.
    # 번역하는 쪽이 이걸 빼야 하나 물어왔고, 확인해보니 우리 쪽에
    # 구멍이 있었다. 번역자가 판단할 일이 아니라 기계가 할 일이다.
    "韓国化粧品", "韓国コスメ", "韓国スキンケア", "正規品", "並行輸入",
    "送料無料", "即納", "あす楽", "ポイント消化", "限定セール",
    # [10회차 번역 피드백] 대괄호 없이 붙은 태그도 걸러야 한다.
    #
    # `[한국코스메]`는 대괄호 제거 규칙에 걸리는데, 대괄호 없이 그냥
    # 붙은 `코스메공식`은 안 걸렸다. 번역하는 쪽이 "이게 셀러 태그인지
    # 제품명 일부인지 확신이 안 선다"고 물어왔고, 확인해보니 우리가
    # 못 거르고 있었다.
    "コスメ公式", "코스메공식", "코스메 공식", "공식몰", "공식스토어",
    "공식판매", "공식판매처", "공식대리점", "본사직영", "직영점",
]
_PROMO_RE = re.compile("|".join(re.escape(w) for w in _PROMO_WORDS))

# 슬래시로 이어붙인 키워드 나열. "제품명 / 진정 / 저자극 / 키트" 처럼
# 뒤에 특징을 잔뜩 붙이는 형태가 많다. 첫 조각만 남긴다.
_SLASH_TAIL_RE = re.compile(r"\s*/\s*")

# 검색어 최대 길이. 성공한 건의 평균이 39자였다.
_QUERY_MAX_LEN = 45
...
def _clean_query(text: str) -> str:
    ...
    # 슬래시 나열은 첫 조각만 쓴다. 단, 첫 조각이 너무 짧으면(브랜드만
    # 남는 경우) 두 번째까지 붙인다.
    if _SLASH_TAIL_RE.search(t):
        parts = [s.strip() for s in _SLASH_TAIL_RE.split(t) if s.strip()]
        if parts:
            t = parts[0]
            if len(t) < 12 and len(parts) > 1:
                t = f"{t} {parts[1]}"

    t = re.sub(r"\s+", " ", t).strip()

    # 그래도 길면 앞에서 자른다. 상품명은 앞쪽에 브랜드와 제품명이 오고
    # 뒤로 갈수록 부가 설명이라, 뒤를 버리는 쪽이 안전하다.
    if len(t) > _QUERY_MAX_LEN:
        cut = t[:_QUERY_MAX_LEN]
        sp = cut.rfind(" ")
        t = (cut[:sp] if sp > _QUERY_MAX_LEN // 2 else cut).strip()
    return t
...
# 취급한다.
# [v7.62.0 대폭 보강] 제형이 다르면 다른 상품이다.
#
# [왜 늘렸나 — 실측 2026-08-16, 재검증 360건 중간점검]
# 카테고리 검사는 이미 있었는데도 오매칭이 175건 중 10건(5.7%) 나왔다.
# 원인을 하나씩 열어 보니 **목록에 없는 제형이 구멍이었다.**
# 목록에 없는 말만 들어 있으면 후보가 "카테고리 없음"으로 판정되어
# 검사가 그냥 통과된다.
#
#   프롬더스킨 글로우 선 "젤"  -> 글로우 미라클선"밤"   (젤·밤 둘 다 없었음)
#   코코이스트 메가 "스틱" 베이스 -> FB22 "파운데이션"   (둘 다 없었음)
#   앤허니 "샴푸&트리트먼트"   -> 딥 모이스처 헤어"오일" (오일이 없었음)
#
# 그룹을 나눌 때 기준은 **"한국 쇼핑몰에서 서로 대체 가능한가"**다.
# 토너와 스킨은 같은 말이라 한 그룹, 세럼과 앰플은 매대가 갈리므로
# 다른 그룹으로 뗀다.
_PRODUCT_CATEGORY_GROUPS = [
    # 기초 — 제형이 다르면 다른 상품
    {"로션", "에멀전", "유액"},
    {"크림"},
    {"세럼", "에센스"},          # 앰플과 분리 — 매대가 갈린다
    {"앰플"},
    {"토너", "스킨", "화장수"},
    {"미스트"},
    {"젤"},
    {"밤"},                      # 클렌징밤은 아래 클렌징 그룹이 먼저 잡는다
    {"오일"},
    {"부스터", "프라이머"},
    # 팩·패드
    {"마스크", "마스크팩", "시트마스크", "팩"},
    {"패치", "패드"},
    # 세안·클렌징
    {"클렌징", "클렌저", "클렌징폼", "폼클렌징", "세안"},
    {"스크럽", "필링", "각질"},
    # 자외선 차단
    {"선크림", "썬크림", "선쿠션", "선스틱", "선세럼", "선밤", "선젤", "선스프레이"},
    # 메이크업
    {"파운데이션", "쿠션", "비비", "씨씨"},
    {"컨실러"},
    {"파우더"},
    {"립스틱", "틴트", "립밤", "립글로스"},
    {"아이라이너", "마스카라", "아이섀도", "아이브로우"},
    # 헤어
    {"샴푸"},
    {"트리트먼트", "헤어팩", "컨디셔너", "린스"},
    {"헤어오일", "헤어세럼", "헤어에센스"},
...
def _score_candidate(cand: dict, known_brand: str, known_volume: str, others: list[dict], original_query: str = "") -> float:
    ...
    if original_query:
        ...
        # [v7.62.0 브랜드 교차검사] 원본 검색어 맨 앞에 한국 브랜드가
        # 붙어 있는데(v7.50.0 이후 대부분 그렇다) 후보의 브랜드가 그것과
        # 전혀 겹치지 않으면 다른 회사 제품이다.
        #
        # [왜 필요한가 — 실측 2026-08-16]
        # known_brand 대조는 이미 있지만, known_brand가 비어 있는 건에는
        # 작동하지 않는다. 그런 건이 아직 2,206건 남아 있다. 실제 사고:
        #   코코이스트 메가 스틱 베이스 -> 피카소 FB22 파운데이션
        #   벤더마 레드카밍 시카       -> 네시픽 프레쉬 시카 클리어 토너
        # 둘 다 브랜드가 아예 다른데 통과했다.
        #
        # [오탐 방지] 같은 회사를 다르게 적는 경우가 많다 — 실측 175건 중
        # 7건이 그랬다(VT/브이티코스메틱, 정샘물/정샘물뷰티,
        # 클레어스/디어클레어스, 리쥬란/리쥬란코스메틱). 한쪽이 다른 쪽에
        # 포함되면 같은 회사로 본다. 이걸 안 하면 멀쩡한 걸 버린다.
        _cb = re.sub(r"\s*\(.*?\)\s*$", "", cand.get("brand") or "").strip().lower()
        _cb = re.sub(r"[\s\-_.]+", "", _cb)
        if _cb and not known_brand:
            _head = original_query.strip().split()
            if _head:
                _ob = re.sub(r"[\s\-_.]+", "", _head[0].lower())
                if (len(_ob) >= 2 and len(_cb) >= 2
                        and _ob not in _cb and _cb not in _ob
                        and _ob not in cand_name.replace(" ", "")):
                    score -= 5.0
