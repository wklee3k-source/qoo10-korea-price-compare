...
COSMETIC_ALLOWED_CATEGORIES = {
    ...
    "120000020",  # 헤어
    "120000023",  # 맨즈뷰티
    # [v7.1.0 제외] 네일(120000021)·향수(120000022)는 색조와 같은 이유로 뺀다.
    #  색상·호수·향으로 갈리는 상품군이라 이름만으로는 같은 제품인지 알 수
    #  없다. 실측 오매칭: '앰버바닐라 세럼 바디크림' -> '로즈 바디크림',
    #  '파워네일 마그넷' -> '파워 마그넷 세팅 스프레이',
    #  '서울 시크릿 블로썸 30ml' -> '소울 시크릿 블라썸'.
    #  이런 건 사진으로도 구분이 어렵고, 색상 하나 틀리면 반품 사유가 된다.
}
...
def run(keyword_ja: str, target_products: int, max_shops: int | None = None, shops_per_keyword: int | None = None, seed_keywords: list[str] | None = None, state_suffix: str | None = None):
    ...
    # [v7.70.0] 짧은 검색어를 먼저 쓴다.
    #
    # [실측 2026-08-16] 검색어 길이별 효율이 4배까지 벌어진다:
    #     1단어  검색어 13,256 → 상품 397  (천개당 29.9건)
    #     2단어  검색어 13,333 → 상품 393  (천개당 29.5건)
    #   3-4단어  검색어 31,660 → 상품 528  (천개당 16.7건)
    #    5+단어  검색어  6,875 → 상품  52  (천개당  7.6건)
    #
    # 긴 검색어는 상품명을 통째로 옮겨온 것이라 그 상품 하나만 걸리고,
    # 이미 방문한 상점으로 이어진다. 짧은 검색어라야 새 상점이 나온다.
    #
    # 그런데 대기열은 3-4단어와 5+단어가 67%를 차지하고 있었다.
    # 그래서 검색어를 641개 소진하는 동안 상품이 1건 늘었다(실측).
    #
    # 정렬만 바꾼다 — 긴 검색어를 버리지는 않는다. 짧은 것이 다 떨어지면
    # 자연히 긴 것 차례가 오고, 그때는 그것도 쓸모가 있다.
    pending_keywords.sort(key=lambda k: len(k.split()))
