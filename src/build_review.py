...
FORM_GROUPS: dict[str, list[str]] = {
    ...
    # 총칭은 '클렌저'만. '클렌징'을 넣으면 '클렌징 오일'과 '클렌징 폼'이
    # 둘 다 총칭으로 잡혀 서로 같은 것이 돼버린다.
    "클렌징총칭": ["클렌저"],
    "클렌징오일": ["클렌징오일"],
    "클렌징밤": ["클렌징밤"],
    "클렌징워터": ["클렌징워터", "미셀라"],
...
# 세정 제품군. '클렌저/클렌징'이라는 총칭은 이 중 무엇과도 어울린다.
CLEANSING_FORMS = {"클렌징폼", "클렌징오일", "클렌징밤", "클렌징워터", "클렌징밀크",
...
def form_status(qoo10_name: str, kr_name: str) -> str:
    ...
    if a & b:
        return "match"
    # 한쪽이 총칭('클렌저')만 말하고 다른 쪽이 구체 제형이면 어긋난 게 아니다.
    if "클렌징총칭" in a and (b & CLEANSING_FORMS):
        return "match"
    if "클렌징총칭" in b and (a & CLEANSING_FORMS):
        return "match"
    return "mismatch"
