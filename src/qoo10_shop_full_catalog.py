...
# 통째로 멈추는 쪽이 훨씬 손해다.
# [v7.65.0 75 -> 40초] 실측으로 낮췄다.
# 상점 하나 수확에 실제로 걸리는 시간을 재보니 9~31초였다(상품 11건인
# 작은 상점 9초, 777건인 큰 상점 31초). 75초는 그보다 두 배 넉넉해서
# 사실상 상한이 걸릴 일이 없었고, 대신 이상이 생긴 상점 하나가
# 배치 전체를 늘어뜨렸다. 40초면 정상 상점은 전부 통과하고
# 비정상만 잘린다.
SHOP_TIME_LIMIT = 40  # 초 (브라우저 기동·페이지 로드까지 포함)
