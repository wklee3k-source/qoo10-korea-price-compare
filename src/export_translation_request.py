...
각 줄을 한국어 상품명으로 번역해 주세요.

**규칙**
1. 출력은 `번호|한국어명` 형식 한 줄씩. 설명·인사·머리말 없이 목록만 주세요.
2. 번호는 입력과 똑같이 유지하세요. 순서를 바꾸거나 줄을 빼지 마세요.
3. 한국 제품이므로 **원래 한국어 상품명을 복원**하는 것이 목표입니다.
   일본어를 직역하지 말고, 그 브랜드가 한국에서 실제로 쓰는 이름으로 쓰세요.
4. 브랜드명은 한국 표기를 쓰세요 (アヌア→아누아, メディキューブ→메디큐브).
5. 용량·수량(50ml, 2매입 등)은 그대로 두세요.
6. **구성·증정 문구(1+1, 2+1, 세트, 증정, 대용량 등)도 빠뜨리지 말고 번역하세요.**
   몇 개를 주는지가 판매 판단에 필요한 정보입니다. 빼지 마세요.
