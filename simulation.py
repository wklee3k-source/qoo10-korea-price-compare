...
def t23_query_noise_stripped():
    ...
    # 한글로 옮겨진 "한국화장품"은 이미 걸러지는데 원문 그대로 남은
    # `韓国化粧品`은 안 걸러지고 있었다. 번역하는 쪽이 "이걸 빼야
    # 하나"고 물어왔고, 확인해보니 우리 쪽 구멍이었다.
    jp = clean("아누아 어성초77토너 500ml / 韓国化粧品")
    check("23-8 일본어 카테고리 태그 제거",
          "韓国化粧品" not in jp and "어성초77토너" in jp, jp)
    jp2 = clean("조선미녀 라이스선크림 50ml 【正規品】 送料無料")
    check("23-9 일본어 판촉 문구 제거",
          "正規品" not in jp2 and "送料無料" not in jp2 and "라이스선크림" in jp2, jp2)

    # [10회차 번역 피드백] 대괄호 없이 붙은 태그도 걸러야 한다.
    # `[한국코스메]`는 대괄호 제거 규칙에 걸리는데 그냥 붙은
    # `코스메공식`은 안 걸렸다. 번역하는 쪽이 "셀러 태그인지 제품명
    # 일부인지 확신이 안 선다"고 물어와 확인한 결과 우리 구멍이었다.
    bare = clean("웰코스 코스메공식 컨퓸 아르간 골드 트리트먼트 200g")
    check("23-10 대괄호 없는 공식 태그 제거",
          "코스메공식" not in bare and "컨퓸" in bare, bare)
...
def t30_translation_skips_foreign():
    ...
    # [09-05 실측] 직역·음차로 채운 건의 검증 통과율은 14%였다.
    # 전체 평균 48.5%의 3분의 1도 안 된다. 비우면 다음 회차에 다시
    # 오지만, 틀린 값은 '번역완료'로 굳어 영영 고쳐지지 않는다.
    check("30-1d 음차로 채우지 말라는 경고",
          "그건 비운 것보다 나쁩니다" in src)
    # [09-06 피드백] "반복적으로 등장하는데 브랜드를 확인 못하는 경우가
    # 쌓이고 있습니다" -> 확인해보니 그 57개가 전부 이미 사전에 있었다.
    # 요청서가 원문만 주고 한글은 안 알려줘서 회차마다 헛되이 검색하고
    # 있었다. 우리가 아는 걸 안 알려준 우리 잘못이다.
    check("30-1e 아는 한글 브랜드를 요청서에 제공",
          "_load_korean_brands" in src and "= {kr}]" in src)
    check("30-1f 그 표기를 쓰라는 안내",
          "그 한글을 그대로 쓰세요" in src)
    # [실측 2026-08-16] 요청서는 뽑는 순간의 지시문이 박제된 파일이다.
    # 09회차에서 규칙을 여러 번 고쳤는데 사장님 손에 있던 파일은 고치기
    # 전 것이라, '음차하지 말라'·'브랜드 한글 제공' 같은 개선이 네 장
    # 동안 전달되지 않았다. 어느 판으로 뽑은 파일인지 보이게 한다.
    check("30-1g 생성 시각·판 번호 표시",
          "_now_kst" in src and "_version" in src)
    check("30-1h 최신 파일을 쓰라는 안내",
          "더 최근에 받은 파일이 있으면" in src)
...
# ---- #46 수확본과 통합본은 포함관계가 아니다 (2026-08-17)
def t46_harvest_vs_merged():
    """데이터 맵을 그릴 때 두 창고의 관계를 잘못 그리지 않도록.

    [실측 2026-08-17] 데이터 맵에서 '수확본 안에 통합본이 들어있다'고
    그렸는데 사장님이 "수확 754,945 통합 8,407 나머지는 어딧지?"라고
    물으셨다. 계산이 안 맞았다.

    실제 관계:
        수확본 고유상품  548,991
          ├ 수확본에만   542,417   아직 안 옮긴 것
          └ 겹침           6,574   ← 통합본에도 있음
        통합본             8,407
          ├ 겹침           6,574
          └ 통합본에만     1,833   발굴이 검색으로만 찾은 것

    **포함관계가 아니라 겹치는 관계다.** 발굴은 검색으로 상품을 찾고
    수확은 상점을 통째로 훑으므로, 서로 못 본 것이 양쪽에 남는다.

    또 수확본 원본은 756,945줄이지만 같은 상품이 여러 상점에 걸려
    있어, 중복을 빼면 548,991건이다. 줄 수와 상품 수를 혼동하면
    안 된다.
    """
...
def t50_collector_input():
    ...
    # [사장님 결정 2026-08-17] 수집기로 한 번 찾아봤는데 없었던 것은
    # 다시 넣지 않는다. 480건 중 118건이 그랬다. 다시 돌려도 결과가
    # 같은데 건당 20초씩 쓴다.
    check("50-6 이미 찾아본 것은 제외", "link_search_exhausted" in src,
          "다시 돌려도 결과가 같다")
    # [실측 2026-08-17] 파일명을 "작업대상_검수페이지_...json"으로 주니
    # 수집기가 "파일명으로 모드를 못 알아봤습니다"를 띄웠다. 수집기는
    # 파일명에서 검색대상/수집대상/통합 중 하나를 찾아 모드를 정한다.
    # 내용이 맞아도 이름이 다르면 안 읽는다.
    check("50-7 파일명에 '통합' 포함", '작업대상_통합_' in src,
          "수집기가 파일명으로 모드를 정한다")
    # [실측 2026-08-17] 화해가 주는 sale 값은 "화해에서 파는지"이지
    # "한국에서 파는지"가 아니다. 749건이 판매중지로 찍혔는데
    # 올리브영·무신사·지그재그 링크가 멀쩡히 살아 있었다.
    # 사장님이 지그재그에서 22,900원에 팔리는 걸 직접 찾아내셨다.
    check("50-8 링크가 있으면 판매중지 표시를 믿지 않음",
          'if not x.get("product_url"):' in src,
          "링크가 살아 있으면 어딘가에서 파는 것이다")
    # 링크가 있어도 사진이 없으면 검수할 재료가 없다
    check("50-9 사진 없으면 링크 있어도 대상", "has_photo" in src)
